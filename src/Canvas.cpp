#include "Canvas.h"
void Canvas::Initialize(const RenderOptions& _Opt)
	{
		_Width = _Opt.Width;
		_Height = _Opt.Height;
		_FPS = _Opt.FPS;
...
void Canvas::DrawNote(const short k, const int y, int h, const unsigned int c)
	{
		if (h > 5) --h;
		if (h < 1) h = 1;
...
void Canvas::DrawNote2(const short k, const int y, int h, const unsigned int c)
	{
		if (h > 5) --h;
		if (h < 1) h = 1;
		unsigned short r=c&0xFF;
...
void Canvas::Draw3Drect(const int x, const int y, const int w, const int h, const unsigned int c,double q)
{
    int d;
    int p[w];
    unsigned short r=((c&0xFF));
...
void Canvas::Draw3Drect2(const int x, const int y, const int w, const int h, const unsigned int c,double q)
{
    int d;
    int p[h];
    unsigned short r=((c&0xFF));
...
void Canvas::DrawRectangle(const int x, const int y, const int w, const int h, const unsigned int c)
	{
		int i;
		//if (x < _Width)
		for (i = y; i < y + h; ++i)
...
void Canvas::DrawNoteRectangle(const int x, const int y, const int w, const int h, const unsigned int c , int t)
	{
		int i,j;
		if(t<1)t=1;
		for (j=0;j!=t;++j)
...
void Canvas::DrawGrayRect(const int x, const int y, const int w, const int h, const unsigned int c , double n)
	{
        unsigned short r=((c&0xFF));
	    unsigned short g=(((c&0xFF00)>>8));
	    unsigned short b=(((c&0xFF0000)>>16));
...
void Canvas::FillRectangle(const int x, const int y, const int w, const int h, const unsigned int c)
	{
		for (int i = x, xend = x + w; i != xend; ++i)
		{
			for (int j = y, yend = y + h; j != yend; ++j)
...
void Canvas::FI(const int x, const int y, const unsigned int c)
	{
